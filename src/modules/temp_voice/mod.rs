...
use serenity::all::{ChannelId, Context, CreateCommand, Ready, UserId};
use slash_command::Voice;
use sqlx::{any::AnyQueryResult, PgPool, Postgres};
use temp_voice::voice_channel_manager::{VoiceChannelMode, VoiceChannelRow};
use temp_voice::VoiceChannelManager;
use zayden_core::SlashCommand;

use crate::Result;

pub fn register(ctx: &Context, ready: &Ready) -> Result<Vec<CreateCommand>> {
    let commands = vec![Voice::register(ctx, ready)?];
